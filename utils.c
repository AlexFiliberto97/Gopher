...
char *getFullPath(struct FilePath fp) {
	...
	if (fullPath == NULL) {
		printf("Errore in getFullPath - malloc\n");
		return NULL;
	}
...
char *getFullPathNoRoot(struct FilePath fp) {
	...
	if (fullPath == NULL) {
		printf("Errore in getFullPathNoRoot - getFullPath\n");
		return NULL;
	}
	char *fullPathNoRoot = memchr(fullPath, '/', strlen(fullPath));
	if (fullPathNoRoot == NULL) {
		printf("Errore in getFullPathNoRoot - memchr\n");
		return NULL;
	}
	fullPathNoRoot = &fullPathNoRoot[1];
	char *fullPathRes = (char *) malloc(strlen(fullPathNoRoot) + 1);
	if (fullPathRes == NULL) {
		printf("Errore in getFullPathNoRoot - malloc\n");
		return NULL;
	}
...
char *getExtension(char *filepath) {
	...
	if (list == NULL) {
		printf("Errore in getExtension - split\n");
	}
...
char *cpyalloc(char *s) {
	...
	if (res == NULL) {
		printf("Errore in cpyalloc - malloc\n");
		return NULL;
	}
	strcpy(res, s);
	return res;
